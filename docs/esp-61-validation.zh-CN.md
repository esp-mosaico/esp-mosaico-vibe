...
## 云端 OTA 与 PSRAM 优化补充验证

操作者已完成网页验证并触发同包升级，Bridge HTTPS 下载、三个组件校验与提交、
应用重启及 ELF/健康检查均已通过。第二轮优化后应用接收写入平均 130.7 KiB/s，
屏幕采样 131.9–138.3 KiB/s；片内静态数据减少 54.8 KiB。三轮口径、最低水位、
剩余擦除/回执等待及最终固件栈余量调整见
[OTA 速率与 PSRAM 优化实测](recovery-ota-performance.zh-CN.md)。
