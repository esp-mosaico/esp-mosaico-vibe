...
Recovery 自更新、USB 两类更新回路及操作者触发的云端 Bridge 更新已通过，
预置包已更新。云端重试发现的间歇性状态交互失败及其他剩余场景
...
- 计划初始预置包的 `factory.bin` 为 1,831,984 字节，固定槽为 1,835,008 字节，
...
### 控制协议优化实施结果（2026-09-22）

- [x] 后台合并上报进度，与开始/提交授权分离，保留旧服务兼容。
- [x] 控制与文件下载复用各自 HTTPS 连接，错误连接销毁。
- [x] 提交结果先持久化，最终回报最多等待 2 秒后重启。
- [x] 应用擦除按镜像长度对齐扇区，数据分区保留完整擦除。
- [x] 故障注入、服务端事务检查及旧服务实机云端更新验证。
- [ ] 部署新版 Bridge 服务后，验证协议 2 的完整线上链路。

详见 [OTA 性能与证据](recovery-ota-performance.zh-CN.md)。
