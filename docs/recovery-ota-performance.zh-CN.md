...
四项改动已实现：普通组件进度由后台合并上报；协议 2 的 `/authorize` 单独处理
开始/提交授权；控制和下载复用各自的 HTTPS 连接；持久化提交结果后最多等待
2 秒回报便重启；应用只擦除镜像覆盖的完整扇区。设备继续兼容旧服务，在提交
边界补齐旧协议要求的阶段确认。协议细节维护在 Recovery README。

实机为同一 Device ID `4553502d49524953010030eda0f4518e`。Recovery 更新操作
`0605faaa-7712-4841-90a5-2302f2a5fadc` 校验新 ELF
`da24c5ce9a8ef8357fff7bf68cdeeef6d706efedd9df3a8a9e4302dafe29bce4`
且健康，Boot ID 为 `14007907387211421134`。随后捕获到一次真实云端更新：

| 指标 | 前一版本成功样本 | 本次成功样本 |
| --- | ---: | ---: |
| 应用有效字节 | 1,259,472 | 1,259,472 |
| 应用擦除字节 | 13,565,952 | 1,261,568 |
| 应用擦除时间 | 7.717 秒 | 3.101 秒 |
| 应用接收/写入阶段 | 8.964 秒 | 9.073 秒 |
| 计划接受至提交完成 | 62.048 秒 | 17.600 秒 |

本次 16:17:54.931 接受计划、16:18:12.531 提交完成，16:18:21 正常应用就绪。
目标 Boot ID `12671786291924841716`，应用 ELF
`e71682afd7d25ba0692769b51257103408ab23fc2cd115917ab487374165fcd0`
匹配，crash_count 为 0。本地持久化云端操作
`1b2a5f37bee9cda896178167f5321d3c` 的结果为 0。bootloader 和分区表哈希未变。
这些是两次成功样本的对比，不能当成不同网络条件下的固定耗时保证。

实机日志明确显示 `control protocol 1`：线上尚未部署新版 Bridge 服务。
新协议 2 已通过主机上的固件故障注入与 Go 服务端路由/事务测试，尚未完成
新版服务部署后的真实云端联调。测试覆盖丢失进度、授权拒绝、迟到进度不撤销
提交、过期授权、取消、HTTP 连接跨路径复用以及最终响应永久阻塞的 2 秒重启。

本次 Recovery 采样最小片内空闲内存为 115,028 字节，PSRAM 为
13,455,156 字节；Bridge 主任务日志栈余量为 5,588 字节。服务端实现提交
`513dfc2`，固件实现提交 `b520c09`。最终 Recovery 原始镜像 1,815,248 字节，
相较前一版增加 2,368 字节，仍剩 19,760 字节容量。
