...
| --- | ---: | ---: | --- |
| `g_iris` | 9,224 B | 9,224 B | 含两个各 4,096 B 的收发帧缓冲 |
...
| **选定常驻符号合计** | **47,576 B** | **23,000 B** | **内部 DRAM 减少 24,576 B** |

按完整链接归档统计，当前 `libesp_iris.a` 的内部 DRAM 为 14,804 B，
TinyUSB 核心 `libespressif__tinyusb.a` 为 9,563 B，ESP-IDF TinyUSB
封装 `libespressif__esp_tinyusb.a` 为 199 B，三者合计 24,566 B。
基线合计为 49,142 B，同样减少 24,576 B。归档统计比上表的选定
符号多计入其余静态变量和对齐开销。
...
| --- | ---: | --- |
| `g_iris.rx_wire`、`g_iris.tx_wire` | 8,192 | 是后续迁移的主要候选，但它们嵌在带非零初始值的 `g_iris` 中；不能给整个结构直接加 `EXT_RAM_BSS_ATTR`。要拆成独立缓冲、保持 USB 回调访问的任务/连接状态在内部，并验证协议、重连、Core Dump 和更新路径。当前未迁移。 |
| `g_iris` 其余状态 | 1,032 | 包含任务句柄、连接与会话状态、锁、设备身份和崩溃状态；USB 回调也读取任务句柄。逐项拆分的复杂度高，收益仅约 1 KB，当前保留内部。 |
| `g_iris_log_storage` | 4,124 | **技术上可以**：现有 `CONFIG_ESP_IRIS_LOG_RING_STORAGE_PSRAM` 会把 4,096 B 环及其 28 B 元数据放入片外 BSS/Core Dump 段，前提是启用 `CONFIG_SPIRAM_ALLOW_BSS_SEG_EXTERNAL_MEMORY`。但 PSRAM、cache 或 MSPI 故障时，崩溃前日志可能无法保存；当前优先保留内部取证可靠性。 |
...
通常的 SPI1 Flash 操作在这种配置下不会关闭 cache，这使协议帧
缓冲迁移在正常应用中更有希望。但崩溃、cache/MSPI 故障和其他可能
...
ESP-Iris 从 CDC FIFO 分段读取，在自己的 4,096 B 缓冲中组装帧，
TX 也支持部分写入后继续排队；TinyUSB 的 `RX >= EP` 约束仍满足。
...
首次慢速 RPC/更新操作会创建 6,144 B 的 service 栈和一份 9,224 B
的 runtime 工作副本，空闲 1 秒后释放；媒体通道使用最多三个按需分配
的 3,840 B 最新块缓冲；`gsp_hello` 现将它们分配在 PSRAM，媒体
发送前仍复制到内部 wire frame，不直接交给 USB DMA。这样镜像活跃时
...
Gateway 与固定 Utils 修订不一致而拒绝操作，故没有与该基线对应的实机
...
不能拿它当作上述总量。上述 ELF 符号比较给出了本次固定 RAM 差值。
Recovery 仍保留 8,192 B CDC 缓冲和 6,144 B 协议任务栈，因为它执行
高吞吐的 OTA/系统更新写入；未经实机写入与恢复闭环验证，不把正常
应用的 2,048 B CDC 配置推广至 Recovery。新增的媒体 PSRAM 选项
也被 Kconfig 限制为无 ESP-Iris Flash writer 的应用，并保持默认关闭。

在当前 `origin/main`（含 ESP-33）上，`hello_world`、`gsp_hello`、
`iris_acceptance` 和 `iris_crash` 均完成 ESP32-S31 构建，构建工具
