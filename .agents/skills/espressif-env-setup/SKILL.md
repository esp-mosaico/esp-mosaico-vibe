...
上述请求不适用本流程；保留已知环境和错误证据，按当前任务范围处理，不套用全新安装步骤。

只有在本 Skill 已经启动全新安装、且由本 Skill 指示的命令发生可识别故障时，才可按 `troubleshooting*.md` 做有限恢复。恢复仅服务于当前安装流程；若无法恢复，记录环境、命令和原始错误，不得扩展为通用排障。
