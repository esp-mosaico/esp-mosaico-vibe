# ESP-Mosaico repository skills

These shared skills are versioned with the workspace in `.agents/skills/`.
Read [AGENTS.md](../../AGENTS.md) for repository-wide rules, then load only the
skills relevant to the task. Developer-facing guides start at the
[documentation index](../../docs/README.md).

## Skill list

| Skill | Purpose |
|-------|---------|
| [`mosaico-ui`](mosaico-ui/SKILL.md) | Create or improve device UI through design confirmation, simulator validation, and device validation, entering where the current uncertainty lies. |
| [`espressif-env-setup`](espressif-env-setup/SKILL.md) | Install a fresh ESP-IDF, ESP-AT, or ESP-ADF environment and verify the first build. |
| [`idf-low-noise-build`](idf-low-noise-build/SKILL.md) | Run low-noise ESP-IDF builds with complete logs and focused failure diagnostics. |
| [`gsp-sim`](gsp-sim/SKILL.md) | Preview GSP apps on PC with sim_bridge by default (esp-gsp 1.4.0). |
| [`mosaico-game-development`](mosaico-game-development/SKILL.md) | Build Raylib-compatible games with same-source RGB565 simulation and ESP-Iris device validation. |
